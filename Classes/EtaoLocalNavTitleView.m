...
@implementation EtaoLocalNavTitleView
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		CGSize size1 = [[NSString stringWithFormat:@"5000米"] sizeWithFont:font];
		NSLog(@"%f",size1.width);
		
		UIImageView *arrow = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"arrow_down.png"]];
		arrow.frame = CGRectMake(frame.size.width/2+size1.width/2+10,10.0f,10,10);
		[btn setTitle:@"5000米" forState:UIControlStateNormal];
...
- (void) setText:(NSString*)text{
	...
	self._arrow.frame = CGRectMake(self.frame.size.width/2+size1.width/2+10,10.0f,10,10);  	
}
/*
