...
- (id) initWithAnnotation:(id <MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
	if (self = [super initWithAnnotation:annotation reuseIdentifier:reuseIdentifier]) {
		...
		self.offsetFromParent = CGPointMake(8, -7); //this works for MKPinAnnotationView
		self.enabled = NO;
...
- (void)adjustMapRegionIfNeeded {
	...
	// 70 指得是地图和top的偏移，zhangsuntai
	//CGFloat pixelsFromTopOfMapView = -(mapViewOriginRelativeToParent.y + self.frame.size.height- 25  - CalloutMapAnnotationViewBottomShadowBufferSize);
	CGFloat pixelsFromTopOfMapView = -(mapViewOriginRelativeToParent.y + self.frame.size.height + 35  - CalloutMapAnnotationViewBottomShadowBufferSize);
...
- (void)drawRect:(CGRect)rect { 
	...
	color = [[UIColor whiteColor] colorWithAlphaComponent:0.9];
