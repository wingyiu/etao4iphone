...
- (void)loadView
{
    ...
    _ctrlsCache = [[NSMutableDictionary alloc]initWithCapacity:10];
...
- (void)loadPage:(NSNumber*)page
{
    ...
    if([[_bodyArray objectAtIndex:[page integerValue]] isEqual:@""]){
        NSString* title = [_keyArray objectAtIndex:[page integerValue]];
        if([_ctrlsCache objectForKey:title]==nil){
            if(_delegate && 
           [_delegate respondsToSelector:@selector(pageForColAtIndexPath:)]){
                ...
                [_ctrlsCache setObject:body forKey:title];
            }
        }
        else{
            body = [_ctrlsCache objectForKey:title];
            CGRect frame = body.view.frame;
            frame.origin.x = frame.size.width * _currentPage;
            body.view.frame = frame;
        }
        
        [_bodyArray replaceObjectAtIndex:[page integerValue] withObject:title];
...
- (void)reloadPage:(NSNumber *)index
{
    ...
    UIViewController* body = nil;
    //删除原有的page
    NSString* key = [_bodyArray objectAtIndex:[index integerValue]];
    body = [_ctrlsCache objectForKey:key];
    [body.view removeFromSuperview];
    [_bodyArray replaceObjectAtIndex:[index integerValue] withObject:@""];
...
- (void)reloadData
{
    ...
    CGSize newSize = CGSizeMake(self.view.frame.size.width * _countOfPage,  self.view.frame.size.height);
    ...
    for(int i=0;i < _keyArray.count;i++){
        ...
        for(j=0; j<_bodyArray.count;j++){
            NSString* head_title = [_keyArray objectAtIndex:i];
            NSString* body_title = [_bodyArray objectAtIndex:j];
    ...
    [_viewBody scrollRectToVisible:CGRectMake(0, 0, _viewBody.frame.size.width, _viewBody.frame.size.height) animated:NO];
